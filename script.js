...
function renderStudentView(res) {
    ...
    try {
        ...
        if(set.LOGO_INSTANSI && el('s-logo-ins')) { el('s-logo-ins').src = set.LOGO_INSTANSI; el('s-logo-ins').style.display='block'; }
        if(set.LOGO_SEKOLAH && el('s-logo-sek')) { el('s-logo-sek').src = set.LOGO_SEKOLAH; el('s-logo-sek').style.display='block'; }

        if(el('res-nama')) el('res-nama').innerText = s.nama || '-';
        if(el('res-nisn')) el('res-nisn').innerText = s.nisn || '-';
        if(s.foto && el('res-foto')) { el('res-foto').src = s.foto; el('res-foto').style.display='block'; }
        ...
        console.error("Crash di Tampilan Siswa:", error);
        Swal.fire('Sistem Error', 'Gagal memuat layar: ' + error.message, 'error');
    }
}
...
function renderStudentResult(res) {
    try {
        ...
        if(el('res-status')) {
            ...
            const sBg = s.status === 'LULUS' ? '#d1fae5' : '#fee2e2';
            const sCol = s.status === 'LULUS' ? '#059669' : '#b91c1c';
            el('res-status').style.cssText = `display:inline-block; padding:6px 15px; border-radius:20px; font-weight:800; background:${sBg}; color:${sCol};`;
        }

        if(el('res-ucapan')) el('res-ucapan').innerText = s.ucapan || (s.status == 'LULUS' ? 'Selamat, Anda Lulus!' : 'Mohon maaf, Anda belum lulus.');
